...
exports.markCheckIn = async (req, res) => {
  ...
  const result = await attendanceService.markCheckIn(userId, latitude, longitude);
  return res.status(result.statusCode).json({ message: result.message, ...(result.data && { data: result.data }) });
};
...
exports.markCheckOut = async (req, res) => {
  ...
  const result = await attendanceService.markCheckOut(userId, latitude, longitude);
  return res.status(result.statusCode).json({ message: result.message, ...(result.data && { data: result.data }) });
};
...
exports.applyForWFH = async (req, res) => {
  ...
  if (!wfhReason) {
    return res.status(400).json({ message: 'Work from home reason is required.' });
  }
